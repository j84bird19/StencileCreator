...
/* ---------- V7 Auto Subject Isolation / Auto Prepare ---------- */
function v7MaskSummary(){
  if(!state.mask) return {keep:0,remove:0,total:0};
  let keep=0,remove=0;
  for(let i=0;i<state.mask.length;i++){
    if(state.mask[i]===1) keep++;
    else if(state.mask[i]===-1) remove++;
  }
  return {keep,remove,total:state.mask.length};
}
function v7BorderColor(){
  const d=state.baseData,w=cvs.width,h=cvs.height,src=d.data;
  let r=0,g=0,b=0,n=0;
  const step=Math.max(3,Math.floor(Math.min(w,h)/160));
  for(let x=0;x<w;x+=step){
    for(const y of [0,h-1]){
      const p=(y*w+x)*4; r+=src[p]; g+=src[p+1]; b+=src[p+2]; n++;
    }
  }
  for(let y=0;y<h;y+=step){
    for(const x of [0,w-1]){
...
function v7Saturation(r,g,b){
  const mx=Math.max(r,g,b),mn=Math.min(r,g,b);
  return mx===0?0:(mx-mn)/mx;
}
function v7Luma(r,g,b){return .299*r+.587*g+.114*b;}
function v7FallbackSkinScore(r,g,b){
  const max=Math.max(r,g,b),min=Math.min(r,g,b);
  const skin=(r>75&&g>40&&b>25&&r>g&&r>b&&max-min>15)?1:0;
  const lightSkin=(r>120&&g>85&&b>65&&r>=g&&g>=b-10)?.75:0;
  return Math.max(skin,lightSkin);
}
function v7SubjectProbability(x,y,r,g,b,bg,mode,strength){
  const w=cvs.width,h=cvs.height;
  const cx=w*.52,cy=h*.48;
  const rx=w*(mode==="portrait"?.43:.48);
  const ry=h*(mode==="portrait"?.56:.52);
  const dx=(x-cx)/rx,dy=(y-cy)/ry;
  const center=Math.max(0,1-(dx*dx+dy*dy));
  const l=v7Luma(r,g,b),sat=v7Saturation(r,g,b);
  const bgDist=(Math.abs(r-bg[0])+Math.abs(g-bg[1])+Math.abs(b-bg[2]))/255;
  const skin=(typeof v6SkinScore==="function")?v6SkinScore(r,g,b):v7FallbackSkinScore(r,g,b);
  const dark=(255-l)/255;
  let score=0;
  if(mode==="portrait"){
    score+=center*.42+skin*.34+dark*.20+sat*.10+Math.min(1,bgDist)*.22;
    if(center<.04&&bgDist<.38&&skin<.15) score-=.42;
    if(y<h*.12&&center<.25) score-=.28;
  }else if(mode==="logo"){
    score+=dark*.58+Math.min(1,bgDist)*.30+center*.16;
    if(l>225) score-=.32;
  }else if(mode==="art"){
    score+=sat*.38+dark*.28+Math.min(1,bgDist)*.22+center*.22;
  }else{
    score+=center*.34+Math.min(1,bgDist)*.34+dark*.22+sat*.18;
  }
  return score+(strength-72)/160;
}
function v7MaskDilate(mask,w,h,rad=2){
  const out=new Int8Array(mask.length);
  for(let y=0;y<h;y++)for(let x=0;x<w;x++){
    let hit=false;
    for(let yy=-rad;yy<=rad&&!hit;yy++)for(let xx=-rad;xx<=rad;xx++){
      const nx=x+xx,ny=y+yy;
      if(nx>=0&&ny>=0&&nx<w&&ny<h&&mask[ny*w+nx]===1){hit=true;break;}
    }
    out[y*w+x]=hit?1:-1;
  }
  return out;
}
function v7MaskErode(mask,w,h,rad=2){
  const out=new Int8Array(mask.length);
  for(let y=0;y<h;y++)for(let x=0;x<w;x++){
    let count=0,total=0;
    for(let yy=-rad;yy<=rad;yy++)for(let xx=-rad;xx<=rad;xx++){
      const nx=x+xx,ny=y+yy;
      if(nx>=0&&ny>=0&&nx<w&&ny<h){total++;if(mask[ny*w+nx]===1)count++;}
    }
    out[y*w+x]=count>=total*.48?1:-1;
  }
  return out;
}
function v7KeepLargestSubject(mask,w,h){
  const seen=new Uint8Array(w*h),comps=[];
  for(let i=0;i<w*h;i++){
    if(mask[i]!==1||seen[i])continue;
    let q=[i],qi=0,pix=[],sx=0,sy=0;seen[i]=1;
    while(qi<q.length){
      const id=q[qi++],x=id%w,y=Math.floor(id/w);
      pix.push(id);sx+=x;sy+=y;
      [1,-1,w,-w].forEach(o=>{
        const ni=id+o,nx=ni%w,ny=Math.floor(ni/w);
        if(ni>=0&&ni<w*h&&Math.abs(nx-x)+Math.abs(ny-y)===1&&mask[ni]===1&&!seen[ni]){
          seen[ni]=1;q.push(ni);
        }
      });
    }
    const cx=sx/pix.length,cy=sy/pix.length;
    const centerBonus=1-(Math.abs(cx-w*.52)/w+Math.abs(cy-h*.48)/h);
    comps.push({pix,score:pix.length*(.7+Math.max(0,centerBonus))});
  }
  comps.sort((a,b)=>b.score-a.score);
  const out=new Int8Array(w*h);out.fill(-1);
  if(!comps.length)return mask;
  const keepCount=Math.min(3,comps.length);
  for(let i=0;i<keepCount;i++){
    if(i===0||comps[i].pix.length>comps[0].pix.length*.08){
      comps[i].pix.forEach(p=>out[p]=1);
    }
  }
  return out;
}
function v7AutoPrepareSubject(){
  if(!state.baseData){status("Upload image first.");return false;}
  pushHistory();
  const d=state.baseData,w=cvs.width,h=cvs.height,src=d.data;
  const mode=$("#autoPrepareMode")?.value||"portrait";
  const strength=+($("#autoIso")?.value||72);
  const bg=v7BorderColor();
  let mask=new Int8Array(w*h);
  for(let i=0;i<w*h;i++){
    const p=i*4,x=i%w,y=Math.floor(i/w);
    const r=src[p],g=src[p+1],b=src[p+2];
    const prob=v7SubjectProbability(x,y,r,g,b,bg,mode,strength);
    const threshold=mode==="portrait"?.43:mode==="logo"?.38:.42;
    mask[i]=prob>=threshold?1:-1;
  }
  if(state.mask){
    for(let i=0;i<mask.length;i++){
      if(state.mask[i]===1)mask[i]=1;
      else if(state.mask[i]===-1)mask[i]=-1;
    }
  }
  mask=v7MaskDilate(mask,w,h,2);
  mask=v7MaskErode(mask,w,h,2);
  mask=v7KeepLargestSubject(mask,w,h);
  mask=v7MaskDilate(mask,w,h,1);
  mask=v7MaskErode(mask,w,h,1);
  state.mask=mask;
  drawOverlay();
  const s=v7MaskSummary();
  status(`Auto prepared subject: kept ${Math.round(s.keep/s.total*100)}%, removed ${Math.round(s.remove/s.total*100)}%.`);
  return true;
}
function v7GenerateWithAutoPrepare(){
  if($("#autoPrepareBeforeGenerate")?.checked){
    const s=v7MaskSummary();
    if(s.keep<s.total*.03||s.remove<s.total*.20) v7AutoPrepareSubject();
  }
  if(typeof v6GenerateLayers==="function") v6GenerateLayers();
  else if(typeof v5GenerateLayers==="function") v5GenerateLayers();
}
if($("#autoPrepareBtn")) $("#autoPrepareBtn").onclick=v7AutoPrepareSubject;

function generate(){ v7GenerateWithAutoPrepare(); }
